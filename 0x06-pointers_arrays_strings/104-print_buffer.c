...
void print_buffer(char *b, int size)
{
	...
	for (offset = 0; offset < size; offset += 10)
	{
		...
		{
			(offset + byte_index < size) ? printf("%02x", b[offset + byte_index]) : printf("  ");
			(byte_index % 2) ? printf(" ") : 0;
		}
		printf(" ");
		for (char_index = 0; char_index < 10; char_index++)
			(offset + char_index < size) ? printf("%c", (b[offset + char_index] >= 32 && b[offset + char_index] <= 126) ?
				b[offset + char_index] : '.') : 0;
		printf("\n");
	}
	(size <= 0) ? printf("\n") : 0;
}
